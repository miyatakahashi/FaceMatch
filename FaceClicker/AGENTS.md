...
- **`index.html`** — Loads Phaser 3.80 via jsDelivr CDN, then `src/GameScene.js` and `main.js` via `<script>` tags (order matters: `main.js` references `GameScene` global, so `GameScene.js` must load first in the HTML).
...
- **`src/GameScene.js`** — **Does not exist yet** (project is a template). Create it with a class named `GameScene` extending `Phaser.Scene`. All game logic lives here.
